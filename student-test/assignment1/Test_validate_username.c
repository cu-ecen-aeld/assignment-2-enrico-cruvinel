...
void test_validate_my_username()
{
     ...
     */
    const char* autotest_username = my_username();
    const char* conf_username = malloc_username_from_conf_file();
    TEST_ASSERT_EQUAL_STRING_MESSAGE(autotest_username, conf_username,"Username from config file and from autotest C file are not equal!");
}
